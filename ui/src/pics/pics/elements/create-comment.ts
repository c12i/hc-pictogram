...
export class CreateComment extends LitElement {
  ...
  async createComment(fields: any) {
    if (this.picHash === undefined)
      throw new Error('Cannot create a new Comment without its pic_hash field');
    ...
    try {
      ...
      const record: EntryRecord<Comment> =
        await this.picsStore.client.createComment(comment);
  ...
  render() {
    return html` <sl-card style="flex: 1;">
      ...
      >
        <div>
          <sl-input name="text" .label=${msg('Text')} required></sl-input>
        </div>
